% This script runs the dynamics of the seasonal food web of the Bialowieza forest with different setups:
...
for Group = {'Agg' 'Bird' 'Rode' 'Ungu'}
	...
	for Rep = 1:NRep
		...
		for Fact = [0.1 0.5]
			...
			dlmwrite(char(strcat('../Outputs/TS_VarDiscRates_Sm2_', DDName, '_DegraInform_Rep', num2str(Rep), '_Dev', num2str(Fact), '_', string(Group), '.csv')), [T Y], 'delimiter', ',');
			clear T Y
		end
	end
end
