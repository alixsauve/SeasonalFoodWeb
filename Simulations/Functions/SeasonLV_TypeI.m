function xdot = SeasonLV_TypeI(tt, xt, SpParam, IntParam, IsStep, IsReproSeasonal)
	% SeasonLV_TypeI() returns the growth rates of species involved in a seasonal food web described by IntParam
	...
	if (isempty(IntParam.LowerTaxon)) | (isempty(IntParam.UpperTaxon)) | (isempty(IntParam.E)) | (isempty(IntParam.G)) | (isempty(IntParam.SeasonDom))
		error('IntParam must have at least 5 columns, namely `LowerTaxon`, `UpperTaxon`, `E`, `G`, and `SeasonDom`.');
	end
...
%					xdot(sp) = xdot(sp) + (SpParam.R(sp) - SpParam.BETA(sp)*xt(sp)) * (1 + ForcingSignalS); % assuming reproduction happens during summer
				...
				for int = int_sp.'
					pred = IntParam.UpperTaxon(int); % predators' names
					predIndexSp = find(ismember(SpParam.Taxon, pred)); % index of preys in SpParam
					if string(IntParam.SeasonDom(int)) == 'S'
						xdot(sp) = xdot(sp) - IntParam.G(int)*xt(predIndexSp)*(1 + IntParam.E(int)*ForcingSignalS) / SpParam.BodyMass(predIndexSp); % if summer interaction
					else
						xdot(sp) = xdot(sp) - IntParam.G(int)*xt(predIndexSp)*(1 + IntParam.E(int)*ForcingSignalW) / SpParam.BodyMass(predIndexSp); % if winter interaction
					end
				end
			else
				...
				for int = int_sp.'
					...
					if string(IntParam.SeasonDom(int)) == 'S'
						if (~SpParam.IsCst(preyIndexSp))
							xdot(sp) = xdot(sp) + CONV_EFF*IntParam.G(int)*xt(preyIndexSp)*(1 + IntParam.E(int)*ForcingSignalS) / SpParam.BodyMass(sp);
						else
							xdot(sp) = xdot(sp) + CONV_EFF*IntParam.G(int)*(1 + IntParam.E(int)*ForcingSignalS) / SpParam.BodyMass(sp);
						end
						if isnan(sp)
							disp([prey preyIndexSp tt xt(preyIndexSp) IntParam.E(int) IntParam.G(int) sp]);
							error("truc");
						end
					else
						if (~SpParam.IsCst(preyIndexSp))
							xdot(sp) = xdot(sp) + CONV_EFF*IntParam.G(int)*xt(preyIndexSp) * (1 + IntParam.E(int)*ForcingSignalW) / SpParam.BodyMass(sp);
						else
							xdot(sp) = xdot(sp) + CONV_EFF*IntParam.G(int)* (1 + IntParam.E(int)*ForcingSignalW) / SpParam.BodyMass(sp);
						end
						if isnan(xdot(sp))
							disp([prey preyIndexSp tt xt(preyIndexSp) IntParam.E(int) IntParam.G(int) sp]);
							error("truc");
